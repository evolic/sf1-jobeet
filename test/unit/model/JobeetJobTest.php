...
$t->is(
  ...
  '->getPositionSlug() return the slug for the position name'
);
$t->is(
  $job->getLocationSlug(),
  Jobeet::slugify($job->getLocation()),
  '->getLocationSlug() return the slug for the location'
);
...
// testing lucene search engine
$jobsTable = Doctrine_Core::getTable('JobeetJob');

$t->comment('->getForLuceneQuery()');
$job = create_job(array('position' => 'foobar', 'is_activated' => false));
$job->save();
$jobs = $jobsTable->getForLuceneQuery('position:foobar');
